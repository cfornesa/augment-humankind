...
class NavigationItem
{
    ...
    private static function legacyPublicItems(): array
    {
        $items = [
            ...
            [
                ...
                'url' => '/portfolio',
                'target' => null,
                'active_key' => 'portfolio',
            ],
        ];

        $systemSlugs = array_flip(self::systemSlugs());
        foreach (Page::navItems() as $page) {
            if (isset($systemSlugs[(string) $page['slug']])) {
                continue;
            }
    ...
    private static function legacyAdminItems(bool $isVisible): array
    {
        ...
        foreach (Page::all() as $page) {
            if (in_array((string) $page['slug'], self::systemSlugs(), true)) {
                continue;
            }
    ...
    private static function removePageItemsForSystemUrls(): void
    {
        $systemSlugs = self::systemSlugs();
        if ($systemSlugs === []) {
            return;

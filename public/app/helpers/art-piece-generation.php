...
function art_piece_generation_system_prompt(string $engine): string
{
    return match ($engine) {
        ...
        'aframe' => implode(' ', [
            ...
            "CRITICAL: Asset `<img>` elements may already be fully loaded (inlined as data URLs) before `window.sketch` runs, so a bare `img.addEventListener('load', ...)` may NEVER fire. Whenever generated JS reads an asset image's naturalWidth/naturalHeight or reacts to its load, ALWAYS use this pattern: `const ready = () => { /* use img.naturalWidth / img.naturalHeight */ }; if (img.complete && img.naturalWidth > 0) ready(); else img.addEventListener('load', ready, { once: true });`.",
            ...
            "CRITICAL: If you author any camera entity (`<a-camera>` or `<a-entity camera>`), it MUST include `look-controls=\"magicWindowTrackingEnabled: false\"` with look-controls left enabled — NEVER set `enabled: false` on look-controls, because that removes all mouse/touch movement for visitors. If the piece needs no click/hover interaction, omit the camera entirely so the default camera (with controls) is used.",

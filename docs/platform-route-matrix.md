...
| `/immersive/pieces/{id}` | implemented in PHP | Renders a full-page immersive piece view. Native Three.js and A-Frame pieces include an edge HUD with a magnifying-glass-labeled vertical zoom slider aligned with the site expand control, directional camera movement, float up/down movement, keyboard movement, pointer/touch gestures, and tap/click-to-move where supported. Desktop/supporting tablet browsers use native fullscreen when available; iPhone Safari uses Immersive Focus instead of relying on native fullscreen. A-Frame's built-in enter-VR/fullscreen button is suppressed so the site expand control is the only expansion control. VR hrefs are built from the same `id` the data endpoint returns; 404s only when the piece genuinely doesn't exist. |

...
| `/immersive/pieces/{id}` | implemented in PHP | Renders a full-page immersive piece view. VR hrefs are built from the same `id` the data endpoint returns; 404s only when the piece genuinely doesn't exist. |

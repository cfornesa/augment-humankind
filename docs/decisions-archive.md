...
<!-- Full record of sessions archived from DECISIONS.md. Do not edit
     retroactively; append-only via archiving passes. Open items were
     carried forward to DECISIONS.md at archive time. -->
...
### Implemented
- **`public/assets/js/admin-piece-capture.js`**: Updated the capture iframe mounting to wrap the iframe inside a `1px` by `1px` `overflow: hidden` parent container `div` styled at `z-index: 999999; opacity: 1;`. The iframe inside the container is kept at its full size (`960x540`) with absolute positioning. This bypasses both WebKit's occlusion culling (as the 1px parent container intersects the viewport on top and is opaque) and avoids the layout-size collapsing bug (where Safari collapses viewport dimensions and breaks WebGL/canvas context when an iframe's layout width/height is scaled below 1px).
- **`tests/three-runtime-consistency.php`**: Updated the test assertions to verify that the parent container features `overflow:hidden`, `opacity:1`, and `z-index:999999`.
...
## 2026-06-29 — Two More Gyro Bugs: Piece-Specific Zoom-In Drag, and Motion Never Actually Enabling on iOS

### Context
The user reported two more issues after the desktop black-out fix: (1) dragging left/right on one specific piece (83) made it zoom in until the figure appeared enlarged, and (2) physically moving/tilting a real phone never actually changed the view, despite that being the entire point of the feature — clarified via a question that the intent (matching A-Frame exactly) is gyro driving rotation only, with drag/wheel/WASD still driving movement on every device by default, desktop included.

**Root cause (1):** the previous fix's "probe for a real sensor" approach worked by constructing a real `DeviceOrientationControls` instance and watching whether real events arrived, discarding the instance if not. But that class's constructor unconditionally calls `camera.rotation.reorder('YXZ')` — a permanent side effect on the shared camera object, regardless of whether the probe ultimately succeeded or failed. This happened on every device, including desktop, on every Three.js immersive piece. The reorder doesn't move the camera at that instant, but it silently changes how *later* rotation math is interpreted — including a piece's own scripted camera animation, which is exactly the kind of code piece 83 runs before/alongside user interaction. Other pieces likely didn't show a visible symptom only because they don't happen to script camera rotation in a way sensitive to Euler order.

**Root cause (2):** on iOS, `DeviceOrientationEvent.requestPermission()` must be called as a direct result of a genuine user gesture (transient activation). The call was being made automatically inside an async mount chain, well after the tap/click that navigated to the immersive page — which essentially never counts as a fresh gesture by the time it runs, so the request reliably failed silently with no fallback. A-Frame's own `device-orientation-permission-ui` solves this exact problem with a manual Allow/Deny button that re-requests permission from inside its own click handler (always a fresh gesture) when the automatic attempt fails — a fallback this implementation never had.

### Implemented
- **`immersive-gallery.js`**: split `setupGyroControls()` into smaller pieces. `waitForRealOrientationData()` now probes for a genuine event using a plain `window.addEventListener('deviceorientation', ...)` — never constructing `DeviceOrientationControls` itself — so the Euler-reorder side effect can no longer happen until a real sensor is already confirmed. `activateGyro()` is the only place that constructs the real controls, called either after a real event is observed (Android/desktop-disambiguation path) or after permission is explicitly granted (iOS path). Added `createEnableMotionButton()`, a manual fallback button mirroring A-Frame's own Allow button: shown when the automatic `requestPermission()` call fails, and re-requests permission from inside its own click handler, which reliably succeeds since a button click is always a fresh gesture.
- Desktop behavior is now provably unaffected by this feature in all cases (no probe-related side effect ever fires without a real sensor); iOS now has the same two-stage permission flow (auto-attempt, then manual fallback button) that already makes A-Frame's equivalent feature work.

### Verification
- `node --check` clean on `immersive-gallery.js`.
- Ran `tests/three-runtime-consistency.php`: 62/62 passed, including the existing dynamic-import regression guards (still valid against the refactored function structure).
- Could not live-verify on real desktop/iOS devices in this session (no browser access) — reasoned from the confirmed mechanisms (the constructor's unconditional `reorder()` call; iOS's transient-activation requirement for `requestPermission()`) rather than fresh screenshots. Both issues need the user's own confirmation: piece 83 dragging normally on desktop, and the "Enable Motion Controls" button actually appearing and working on a real iPhone.

---

## 2026-06-29 — Three.js Gyro: Dialog CSS in the Wrong File, Stale Cache, and a Render-Gate the Feature Never Joined

### Context
Live testing on a real iPhone surfaced three more issues, traced precisely rather than patched on guesses (the user explicitly pushed back on a prior turn for overpromising without verification — this entry's findings were each confirmed via direct code-reading/tracing before being claimed fixed):

1. **The A-Frame permission dialog was still unreadable** — the screenshot showing it broken was actually `/pieces/{id}` (the regular public piece page, confirmed by its exact layout), not the `/immersive/pieces/{id}` view this session's earlier dialog fix targeted. `/pieces/{id}` embeds the piece via `piece_render_iframe()` → `piece_render_document()` (`piece-render.php`) — a separate standalone HTML document with no `--panel-bg`/`--text-primary` variables at all. The earlier fix's `var(--panel-bg)` reference was invalid there (undefined custom property, no fallback), so the cascade fell through to A-Frame's own hardcoded white card every time. This document is the shared runtime for the public piece page, embeds, admin previews, and thumbnail capture — higher-traffic than the two immersive views the original fix covered.
2. **No permission gate appeared at all on `/immersive/pieces/83`.** Re-read `setupGyroControls()` end to end and confirmed it has no silent no-op path; confirmed the deploy pipeline wasn't the problem either (`git status` clean, `origin/main` == `HEAD`, GitHub Actions `deploy.yml` shows `completed`/`success` for the relevant commits). The actual cause: `immersive/piece.php` and `immersive/collection.php` import `immersive-gallery.js` with no cache-busting query string — a failure mode this codebase already has a documented, proven fix for on a sibling file (`piece-render.php`'s `?v=<?= $runtimeVersion ?>` on `piece-runtime.js`, specifically because "browsers (WebKit/Safari especially) can keep serving a stale cached copy ... indefinitely after a deploy"). That fix was never extended to `immersive-gallery.js`, which had been redeployed many times this session.
3. **After a manual refresh got the dialog working, tilting still did nothing.** Traced this to a specific existing code path rather than guessing: `animateControls()` only calls `state.renderer.render(...)` when `!pieceDrivesOwnRender || userHasInteracted` — many AI-generated Three.js pieces script their own per-frame camera/render loop. `userHasInteracted` already exists specifically to let real user input take over from a piece's scripted camera (set from `isOrbitActive || externalMotion`, i.e. drag or keyboard), but gyro activity was never wired into it. `deviceControls.update()` was correctly writing the tilted orientation onto `state.camera.quaternion` every frame — it just never reached the screen, because a piece with `pieceDrivesOwnRender = true` would silently overwrite that quaternion with its own scripted camera before our render call would ever execute (which itself stayed skipped, since `userHasInteracted` stayed false). Confirmed the user's own A-Frame piece does rotate correctly on the same iPhone first, ruling out a WebXR/positional-AR ceiling and confirming the 3DoF rotation goal is reachable on this device — narrowing the bug to this specific render-gate, not an architectural limit.

### Implemented
- **`piece-render.php`**: added `.a-dialog`/`.a-dialog-text`/`.a-dialog-button` overrides inside `piece_render_document()`'s `<style>` block, using literal hardcoded colors (`#162a3a` panel, `#dde8ef` text, `#15374a` button text) since this document has no theme variables or toggle of its own — same values already established for the immersive-view fix, just hardcoded here instead of referencing custom properties that don't exist in this context.
- **`immersive/piece.php`, `immersive/collection.php`**: added `$galleryRuntimeVersion = (int) @filemtime(...)` and appended `?v=<?= $galleryRuntimeVersion ?>` to each file's `immersive-gallery.js` import, mirroring `piece-render.php`'s existing pattern exactly.
- **`immersive-gallery.js`**: `activateGyro()` now also sets `userHasInteracted = true` when gyro successfully activates — tilting the phone to look around is taking control of the camera exactly as much as a drag or key press already counts as, so it now joins the same existing takeover mechanism instead of being silently overwritten by a piece's own scripted render loop.

### Verification
- `php -l` clean on `piece-render.php`, `immersive/piece.php`, `immersive/collection.php`; `node --check` clean on `immersive-gallery.js`.
